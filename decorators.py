...
class Car(DomainModel):
    """Все что было выше теперь можно поместить в отдельный пакет
       Далее используем нашу модель предметной области с типами полей
    """
    id = Int()
    title = String()
    manufacturer = String()
    ...
    # Это у нас уникальные поля
    __unique_key__ = (id, manufacturer)
    # Поля для вывода
    __view_key__ = (id, title, manufacturer)
...
class Dealer(DomainModel):
    ...
    city = String()
    phone = String()
    # У диллера есть коллекция машин аналог ForeignKey в Django
    cars = FieldCollection(Car)




# При создании объктов значения будут конвертированы в нужный тип
car_1 = Car(
    id=1,
    title='A3',
    manufacturer='Audi',
    ...
    price=25876.8,
    discount=0,
    production_date=datetime.date(year=2018, month=2, day=5),
    available=True
)
...
car_2 = Car(
    ...
    price=35517.2,
    discount=0,
    production_date=datetime.date(year=2017, month=12, day=20),
    available=True
)
...
car_3 = Car(
    ...
    price=21428.2,
    discount=2.5,
    production_date=datetime.date(year=2018, month=3, day=11),
    available=False
)
...
print(car_3 == car_3_double)
print(car_1.__slots_optimization__)
print(car_1.__slots__)
print(car_1)
...
dealer_1 = Dealer(
    ...
    city='Челябинск',
    phone='+79995551111',
    # У диллера машины передаем списком
    cars=[car_1, car_2, car_3]
)

# OneToOne, ForeignKey (1 - *), * - *

print(dealer_1.get_data().keys())
print(dealer_1.get_data())

print(dealer_1.get_data()['cars'][0].keys())
print(dealer_1.get_data()['cars'][0])
